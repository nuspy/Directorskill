"""Load LitMedia API credentials.

Priority order:
1. Environment variables LITMEDIA_UID + LITMEDIA_API_KEY  (CI / backwards compat)
2. Credential file ~/.LitMedia/credentials.json           (set by auth.py login)
...
def load_config() -> dict:
    ...
    uid = os.environ.get("LITMEDIA_UID", "").strip()
    api_key = os.environ.get("LITMEDIA_API_KEY", "").strip()
    ...
    missing_env = []
    if not uid:
        missing_env.append("LITMEDIA_UID")
    if not api_key:
        missing_env.append("LITMEDIA_API_KEY")

    print(
        "Error: LitMedia credentials not found.\n\n"
        ...
        '  export LITMEDIA_UID="<your-litmedia-uid>"\n'
        '  export LITMEDIA_API_KEY="<your-api-key>"\n',
        file=sys.stderr,
    )
    sys.exit(1)
